...
function Tplayer(Element, src, poster, server, videoid, videotype) {
    ...
    tp.changersound = function() {
        tp.ele.video.volume = parseInt(tp.ele.tp_s.style.width) * .01;
        var Days = 7;
        var exp = new Date();
        exp.setTime(exp.getTime() + Days * 24 * 60 * 60 * 1e3);
        document.cookie = "tpsound=" + parseInt(tp.ele.tp_s.style.width) + ";expires=" + exp.toGMTString() + "&path=/";
    };
    tp.soundcookie = getCookie("tpsound");
    if (tp.soundcookie) {
        tp.ele.tp_s.style.width = tp.soundcookie + "%";
        tp.ele.video.volume = parseInt(tp.ele.tp_s.style.width) * .01;
    } else {
        tp.ele.tp_s.style.width = "80%";
        tp.changersound();
    }
    //获取视频总时间
    ...
    getallvideotime();
    //音量调节
    tp.ele.tp_s_w.addEventListener("click", function(event) {
        var e = event || window.event || arguments.callee.caller.arguments[0];
        var xbl = show_coords(e, this).xbl * 100;
        tp.ele.tp_s.style.width = xbl + "%";
        tp.changersound();
    }, false);
    //定时器
    function danmutime() {
    ...
    document.addEventListener("keydown", function(event) {
        ...
        if (tp.ele.tp_video_warp.xz == true) {
            ...
            if (e && e.keyCode == 38) {
                // up 键
                event.preventDefault();
                tp.ele.tp_s.style.width = parseInt(tp.ele.tp_s.style.width) + 1 + "%";
                tp.changersound();
            }
            if (e && e.keyCode == 40) {
                // down 键
